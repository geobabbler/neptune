...
const extractImageFromHtml = (htmlContent, baseUrl) => {
  ...
  try {
    ...
    // Priority 2: WordPress featured image (common when RSS excerpt omits media)
    const wpFeatured =
      $('.wp-post-image').first().attr('src') ||
      $('img.attachment-post-thumbnail').first().attr('src') ||
      $('.single-featured-image img').first().attr('src');
    if (wpFeatured) {
      const resolved = resolveImageUrl(wpFeatured, baseUrl);
      if (resolved && isValidImageUrl(resolved)) {
        return resolved;
      }
    }

    // Priority 3: first <img>
...
/** Fetch og:image (or WordPress featured image) from the item's HTML page. */
async function fetchOgImageFromPage(pageUrl, opts = {}) {
  if (!pageUrl || typeof pageUrl !== 'string') return null;
  const timeout = opts.timeout ?? CONFIG.FEED_HTTP_TIMEOUT_MS;
  try {
    const response = await axios.get(pageUrl.trim(), {
      timeout,
      headers: {
...
/**
 * For items with no RSS/Atom image, try the post page (og:image / WordPress featured image).
 * Skips items that already have imageUrl.
 */
async function enrichItemsMissingImages(items, opts = {}) {
  if (!CONFIG.FEED_PAGE_IMAGE_FETCH || !Array.isArray(items) || items.length === 0) {
    return items;
  }

  const concurrency = opts.concurrency ?? CONFIG.FEED_PAGE_IMAGE_CONCURRENCY;
  const fetchPageImage = opts.fetchPageImage || fetchOgImageFromPage;
  const missing = items
  ...
  await mapWithConcurrency(missing, concurrency, async ({ item, index }) => {
    const imageUrl = await fetchPageImage(item.link);
    if (imageUrl) {
      enriched[index] = { ...item, imageUrl };
    }
  });
...
/**
 * Stable key for merge/dedup across refreshes. Not intended for MCP wire format.
 * Priority: RSS guid or Atom id (from original), else normalized absolute link, else content hash.
 * Collision handling: first match wins; guid/link are treated as authoritative for the same entry.
 */
const extractGuidOrIdFromOriginal = (original) => {
  if (!original) return null;
  if (original.guid && original.guid[0] != null) {
    const g = original.guid[0];
    if (typeof g === 'string') {
      const s = g.trim();
      return s || null;
    }
    if (g && typeof g === 'object' && typeof g._ === 'string' && g._.trim()) {
      return g._.trim();
    }
  }
  if (original.id && original.id[0] != null) {
    const id = original.id[0];
    if (typeof id === 'string') {
      const s = id.trim();
      return s || null;
    }
    if (id && typeof id === 'object' && id._ != null) {
      const s = String(id._).trim();
      return s || null;
    }
  }
  return null;
...
const computeItemKey = (item) => {
  if (!item) return '';
  const fromGuid = extractGuidOrIdFromOriginal(item.original);
  if (fromGuid) return `guid:${fromGuid}`;
  const L = normalizeLinkKey(item.link);
  if (L) return `link:${L}`;
  const h = crypto
    .createHash('sha256')
    .update(`${item.title || ''}|${item.pubDate || ''}|${item.link || ''}`)
    .digest('hex')
    .slice(0, 40);
  return `hash:${h}`;
};
